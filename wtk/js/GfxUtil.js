...
fan.fgfxWtk.GfxUtil.doJsPath = function(cx, path)
{
  ...
  for (var i =0; i < size; ++i)
  {
    ...
    if (s instanceof fan.fgfxGraphics.PathMoveTo)
    {
      cx.moveTo(s.m_x, s.m_y);
    }
    else if (s instanceof fan.fgfxGraphics.PathLineTo)
    {
      cx.lineTo(s.m_x, s.m_y);
    }
    else if (s instanceof fan.fgfxGraphics.PathQuadTo)
    {
      cx.quadraticCurveTo(s.m_cx, s.m_cy, s.m_x, s.m_y);
    }
    else if (s instanceof fan.fgfxGraphics.PathCubicTo)
    {
      cx.bezierCurveTo(s.m_cx1, s.m_cy1, s.m_cx2, s.m_cy2, s.m_x, s.m_y);
    }
    else if (s instanceof fan.fgfxGraphics.PathClose)
    {
      cx.closePath();
    }
    else
