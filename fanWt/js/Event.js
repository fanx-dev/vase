...
fan.fanWt.Event.prototype.sync = function(self, w, h)  // w,h override
{
  // sync event handlers
...
fan.fanWt.Event.prototype.checkEventListener = function(self, mask, type, evtId, listeners)
{
...
fan.fanWt.Event.prototype.attachEventListener = function(self, type, evtId, listeners)
{
  var peer = this;
  var func = function(e)
  {
    ...
    evt.m_widget = self;
    evt.m_key    = fan.fanWt.Event.toKey(e);
    if (isClickEvent)
    {
      evt.m_button = e.button + 1;
      evt.m_count  = fan.fanWt.Event.processMouseClicks(peer, evt);
    }
    if (isWheelEvent)
    {
      evt.m_button = 1;  // always set to middle button?
      evt.m_delta = fan.fanWt.Event.toWheelDelta(e);
    }
...
fan.fanWt.Event.processMouseClicks = function(peer, e)
{
...
fan.fanWt.Event.toWheelDelta = function(e)
{
  var wx = 0;
  var wy = 0;
...
fan.fanWt.Event.toKey = function(event)
{
  // find primary key
  var key = null;
  if (event.keyCode != null && event.keyCode > 0)
  {
    ...
    key = fan.fanWt.Event.keyCodeToKey(code);
  }
...
fan.fanWt.Event.keyCodeToKey = function(keyCode)
{
