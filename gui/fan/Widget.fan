...
//////////////////////////////////////////////////////////////////////////
  ...
  Animation fadeInAnim(Duration time := 500ms) {
    a := TweenAnimation {
      it.duration = time.toMillis
      AlphaAnimChannel { to = 1f; from = 0f; },
    }
    a.bind(this)
    return a
  }
  Animation fadeOutAnim(Duration time := 500ms, Bool detach := true) {
    a := TweenAnimation {
      it.duration = time.toMillis
  ...
  Animation moveInAnim(Direction orig, Duration time := 500ms) {
    x := 0
    y := 0
    p := Coord(0, 0)
    posOnWindow(p)
  ...
  Animation moveOutAnim(Direction orig, Duration time := 500ms, Bool detach := true) {
  ...
  Animation shakeAnim(Duration time := 100ms, Int repeat := 10) {
    a := TweenAnimation() {
      it.repeat = repeat
      it.duration = time.toMillis
  ...
  Animation shrinkAnim(Duration time := 500ms, Coord? p := null, Bool detach := true) {
    if (p == null) {
      p = Coord(0, 0)
      this.posOnWindow(p)
    }
  ...
  Animation expandAnim(Duration time := 500ms, Coord? p := null) {
    if (p == null) {
      p = Coord(0, 0)
      this.posOnWindow(p)
    }
  ...
  Animation scaleAnim(Float from := 0f, Duration time := 500ms) {
    a := TweenAnimation() {
      it.duration = time.toMillis
